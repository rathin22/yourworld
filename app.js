...
const AllPlaces = JSON.parse(fs.readFileSync('AllPlaces.json', (err, data) => {
...
function GetAllFilePaths () { // Creates dictionary with keys as place names and a list of their image names as value
    const AllFilePaths = {};
    for (const place of Object.keys(AllPlaces)) {
...
app.get('/place/images/:placename', function (req, resp) { // Returns list of image names of specified place
    const images = fs.readdirSync('client/userimages/' + req.params.placename);
    resp.json(images);
});
...
app.get('/allpictures', (req, resp) => { // returns list of all pictures currently uploaded
    ...
    for (const place of Object.keys(AllFilePaths)) {
        for (const image of AllFilePaths[place]) {
